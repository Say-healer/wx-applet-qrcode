...
!(function () {
  ...
  var api = {
    ...
    drawQrcode(options) {
      ...
      ctx.save()
      this.draw(options.text, ctx, options.x, options.y, options.width, options.height, options.background, options.foreground, options._this, options.correctLevel);

      if (options.image.src) {
        options.image.dx = (options.width / 2) + options.x - (options.image.dWidth  / 2)
        options.image.dy = (options.height / 2) + options.y - (options.image.dHeight  / 2)
        ctx.drawImage(options.image.src, options.image.dx, options.image.dy, options.image.dWidth, options.image.dHeight)
      }
      ctx.restore()
      options.callback && options.callback(ctx)
